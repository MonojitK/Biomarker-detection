...
with open("../utilities/pathway_utilities.py") as f:
    ...
    exec(code, globals())
with open("../utilities/parse_Drugbank.py") as f:
    code = compile(f.read(), "../utilities/parse_Drugbank.py", 'exec')
    exec(code, globals())
...
def parse_organoid_ssGSEA_NES(cancer_type, pathway_source):
	...
	----
	Input
	cancer_type = 'LUAD', 'blca'
	pathway_source = 'reactome'
	'''
	if cancer_type.lower() == 'coad':
		with open("../utilities/GESA.py") as f:
			code = compile(f.read(), "../utilities/GESA.py", 'exec')
			exec(code, globals())
...
def parse_coad_organoid_transcriptome():
	...
	with open("../utilities/LUAD_Organoid.py") as f:
		code = compile(f.read(), "../utilities/LUAD_Organoid.py", 'exec')
		exec(code, globals())
