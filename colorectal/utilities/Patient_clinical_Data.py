...
def parse_TCGA_drug_treatment_data(cancer_type):
	...
	if 'coad' in cancer_type.lower():
		df = pd.read_csv('../data/TCGA_COAD/clinical_drug/dae92461-d098-40a3-997e-1a2d3e5b8dfe/nationwidechildrens.org_clinical_drug_coad.txt',sep='\t')
		for i in range(len(df)):
			pat, drug = df['bcr_patient_barcode'][i], df['pharmaceutical_therapy_drug_name'][i].replace('-','').replace(' ','')
			drugs = drug.split('AND')
			for drug in drugs:
				if (not 'unknown'.upper() in drug.upper()) and (not 'notavailable'.upper() in drug.upper()):
					common_drug = drug.upper()
					if drug in Syn_common:
						common_drug = Syn_common[drug.upper()]
...
def parse_TCGA_COAD_drug():
	...
	for i in range(len(df)):
		pat, drug = df['bcr_patient_barcode'][i].upper().replace('-','').replace(' ',''), df['pharmaceutical_therapy_drug_name'][i].upper().replace('-','').replace(' ','')
		if ('TCGA' in pat):
			common_drug = drug
...
def parse_TCGA_COAD_survival():
	...
	df = pd.read_csv(fi_directory, sep='\t')
	for i in range(len(df)):
