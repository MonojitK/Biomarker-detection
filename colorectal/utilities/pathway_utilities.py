...
def reactome_genes_ensemble():
    output, reactome = defaultdict(list), reactome_genes()
    gene2ensemble=geneID2ensemble()
    for pathway in reactome:
        for gene in reactome[pathway]:
            if gene in gene2ensemble:
                ensemble = gene2ensemble[gene]
                if not ensemble in output[pathway]:
                    output[pathway].append(ensemble)
    return output
gene2ensemble = geneID2ensemble()
